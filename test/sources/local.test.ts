...
describe("sources/local", () => {
	...
	beforeEach(() => {
		tmpRoot = mkdtempSync(path.join(os.tmpdir(), "gitgud-local-"));
	});
	...
	test("installs a valid local skill and writes meta", () => {
		...
		expect(existsSync(path.join(installedDir, "SKILL.md"))).toBe(true);

		const metaPath = path.join(installedDir, ".gitgud-meta.json");
		...
		expect(new Date(meta.installedAt).toString()).not.toBe("Invalid Date");
	});
	...
	test("errors when source does not exist", () => {
		...
		if (!res.ok) {
			expect(res.error.message).toContain("Source does not exist");
		}
	});
