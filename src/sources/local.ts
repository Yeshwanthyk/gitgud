...
export function installFromLocal(
  ...
  try {
    ...
    return err(
      new Error(`Source does not exist: ${absoluteSource}`),
    );
  }

  if (!srcStats.isDirectory()) {
    return err(
      new Error(`Source is not a directory: ${absoluteSource}`),
    );
  }
  ...
  if (existsSync(destDir)) {
    return err(
      new Error(`Target skill already exists: ${destDir}`),
    );
  }
  ...
  try {
    ...
    return err(
      new Error(
        `Failed to copy skill from ${absoluteSource} to ${destDir}: ${message}`,
      ),
    );
  }
  ...
  try {
    ...
    return err(
      new Error(
        `Failed to write .gitgud-meta.json: ${message}`,
      ),
    );
  }
