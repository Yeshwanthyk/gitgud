...
export async function installCommand(args: string[], options: InstallOptions): Promise<void> {
	...
	if (!sourceInput) {
		process.stderr.write(`${formatError("Missing install source.", options.format)}\n`);
		process.exit(1);
	}
	...
	if (!targetDir) {
		process.stderr.write(`${formatError("Local skills directory not found.", options.format)}\n`);
		process.exit(1);
	}
	...
	try {
		...
		process.stderr.write(`${formatError(message, options.format)}\n`);
		process.exit(1);
	}
	...
	try {
		if (parsed.type === "local") {
			...
			if (!res.ok) {
				process.stderr.write(`${formatError(res.error.message, options.format)}\n`);
				process.exit(1);
			}
