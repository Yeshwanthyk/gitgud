...
export async function runPathCommand(args: string[], options: PathCommandOptions): Promise<void> {
	...
	if (!resolved.ok) {
		process.stderr.write(`${formatError(resolved.error.message, options.format)}\n`);
		process.exit(1);
	}
